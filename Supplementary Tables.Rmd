...
output: 
  word_document:
    reference_docx: Supplementary_Tables.docx
---
...
table.data <- merge(merge(means, sds), t.test.results_timepoint %>% 
  ...
  select(-t, -p, -d.magnitude, -d.method, -lower95CI, -upper95CI) %>%
  ...
  select(mood, contrast, descrip_contr.1, descrip_contr.2, df, t.rounded, p.rounded, p.adjusted, d, CI) %>%
  arrange(mood, contrast)
names(table.data) <- c("Rating", "Time\\\npoints\\\ncontrasts", "First contrast\\\n(SD)", "Second contrast\\\n(SD)", "*df*", "|*t*|", "*p*", "*p* corrected", "*d*", "*d* 95% CI")
...
table.data <- merge(table.data_arousal, table.data_valence, all = T) %>% #arrange(rating) %>%
  ...
  mutate( descrip_excited = paste0(sprintf("%.2f", M.excited), ' (', sprintf("%.2f", SD.excited), ')'),
          ...
          CI = paste0(d.lower95CI, ', ', d.upper95CI)) %>%
  select(rating, block, descrip_excited, descrip_sad, df, t.rounded, p.rounded, p.adjusted, d, CI) 
names(table.data) <- c("Rating", "Time\\\npoint", "Excited\\\n(SD)", "Sad\\\n(SD)", "*df*", "|*t*|", "*p*", "*p* corrected", "*d*", "*d* 95% CI")
