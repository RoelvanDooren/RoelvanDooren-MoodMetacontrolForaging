...
packages <- c("plyr", "zeallot", "Rmisc", "ggplot2", "cowplot", "MBESS", "rstatix", "psych", "ggpubr", "report", "lsmeans", 
              "effsize", "purrr", "gtools", "ez", "apaTables", "dplyr", "data.table", "car", "effsize") # Make sure to load dplyr after plyr and ggplot2
ipak(packages)
...
#### Test wrappers ####
wrapper.t.test <- function(formula, df, equalVariances = T, pairedComparison = F, direction = 'two.sided') {
  t = t.test(formula, df, var.equal = equalVariances, paired = pairedComparison, alternative = direction)
  d <- cohen.d(formula, df)
  data.frame(t=t$statistic, df=t$parameter, p=t$p.value, d=d$estimate, d.magnitude=d$magnitude, d.method=d$method, lower95CI=d$conf.int[1], upper95CI=d$conf.int[2], direction = direction )
}
...
# Perform two seperate repeated-measures ANOVA on participants' affect ratings, separately for arousal and valence ratings, with :
    ...
    map_df(function(x) {
      map_df(combos, function(y) { 
        filter(x, block %in% y) %>% 
          ...
          do(wrapper.t.test(., formula = rating ~ block, pairedComparison = T))
...
# whether participants in the sad conditions significanty decrease in arousal/valence from timepoint 3 to 4.
mood_data_long %>% filter(condition == 'Excited') %>%
  ...
  map_df(function(x) {
    map_df(combos, function(y) { 
      filter(x, block %in% y) %>% 
        droplevels() %>% 
        do(wrapper.t.test(., formula = rating ~ block, direction = 'less', pairedComparison = T))
    }, .id = "contrast") %>% arrange(contrast)
  }, .id = "mood") %>%
  mutate(t.rounded = round(t, 2), p.rounded = round(p, 3)) %>%
...
mood_data_long %>% filter(condition == 'Sad') %>%
  ...
  map_df(function(x) {
    map_df(combos, function(y) { 
      filter(x, block %in% y) %>% 
        droplevels() %>% 
        do(wrapper.t.test(., formula = rating ~ block, direction = 'greater', pairedComparison = T))
    }, .id = "contrast") %>% arrange(contrast)
  }, .id = "mood") %>%
  mutate(t.rounded = round(t, 2), p.rounded = round(p, 3)) %>%
  ungroup() %>% mutate(p.adjusted = round(p.adjust(p.rounded, method = "holm"), 3)) %>% filter(contrast == '3-4')
...
png('./plots/Figure-3.png', width = 1000)
ggplot(data = mood_data_long, aes(x = block, y = rating, group = condition)) + 
  geom_rect(aes(x = block, y = rating, xmin = 3, xmax = 4, ymin = -2, ymax = 4), fill = 'grey30', alpha = .3) + # Indicate where the mood-manipulation occured
  geom_rect(aes(x = block, y = rating, xmin = 1, xmax = 2, ymin = -2, ymax = 4), fill = 'grey80', alpha = .3) + # Indicate where the pre-mood foraging task occured
  geom_rect(aes(x = block, y = rating, xmin = 5, xmax = 6, ymin = -2, ymax = 4), fill = 'grey80', alpha = .3) + # Indicate where the post-mood foraging task occured
  stat_summary(fun.y = mean, geom = "point", aes(color = condition, shape = condition, size = condition)) + # Plot the mean ratings
  stat_summary(fun.y = mean, geom = "line", aes(color = condition, linetype = condition)) + # Plot a line that connects the dots
  ...
  coord_cartesian(ylim = c(-2, 4), xlim = c(1, 6), clip = 'off') + 
  scale_y_continuous(expand = c(0, 0), breaks = seq(-2, 4, 1)) +
  labs(y = "Rating", x = "Time point") +
  stat_compare_means(label = "p.signif", method = "t.test", paired = F, hide.ns = TRUE, label.y = 3.25, size = 8,
                     symnum.args = list(cutpoints = c(0, 0.001, 0.01, 0.05, 1), symbols = c("***", "**", "*", "ns"))) +
  facet_wrap(~ mood) +
  theme( plot.margin = unit(c(3, 1, 1, 1), 'cm'),
         legend.position = c(0.475, 1.2),
